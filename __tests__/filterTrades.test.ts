...
describe('filterTrades',()=>{
  ...
  it('filters sales independently',()=>expect(filterTrades(mockTrades,{query:'',type:'sale',role:'all',minimumValue:0})).toHaveLength(3));
  it('keeps officers under All roles only',()=>{expect(filterTrades(mockTrades,{query:'',type:'all',role:'all',minimumValue:0}).some(x=>x.role==='Officer')).toBe(true);expect(filterTrades(mockTrades,{query:'',type:'all',role:'CEO',minimumValue:0}).some(x=>x.role==='Officer')).toBe(false)});
  it('applies the one-million threshold inclusively',()=>expect(filterTrades(mockTrades,{query:'',type:'all',role:'all',minimumValue:1_000_000}).map(x=>x.id)).toEqual(['nova-01','volt-03','orbt-07']));
});
