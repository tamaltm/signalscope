...
export function TradeCard({ trade, onPress }: { trade: InsiderTrade; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`Open ${trade.company} ${trade.type} details`} onPress={onPress} style={({pressed})=>[styles.card,pressed&&styles.pressed]}>
    <View style={styles.row}><View style={styles.symbol}><Text style={styles.symbolText}>{trade.ticker}</Text></View><View style={styles.identity}><Text numberOfLines={1} style={styles.company}>{trade.company}</Text><Text style={styles.meta}>{trade.insider} · {trade.role}</Text></View><View style={styles.strength}><Text style={styles.strengthText}>{trade.signalStrength}</Text></View></View>
